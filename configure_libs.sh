  ...
  do
    if [[ "$lib" == "glpk" ]]; then
      ...
      if [ ${CHECK:-0} -eq 0 ]; then
        ...
        if [[ "$lib" == "glpk" ]]; then
          GLPK_LIB=$(find -L /usr/lib /usr/local/lib /Library $HOME -name libglpk.a -print0 2>/dev/null | head -1)
    ...
    if [[ "$lib" == "gurobi" ]]; then
      ...
      if [ ${CHECK:-0} -eq 0 ]; then
        ...
        if [[ -z "$GUROBI_HOME" ]]; then
          GUROBI_LIB=$(find -L /usr/lib /usr/local/lib /Library $HOME $CURRENT_DIR -path */lib/libgurobi_c++.a -print0 2>/dev/null | head -1)
